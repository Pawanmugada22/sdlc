...
CREATE TABLE ACC_PER_TASK_COM (
    APTC_TASK_CODE SERIAL,
    APTC_TASK_PER_CODE INT,
    APTC_TASK_NAME VARCHAR(30),
    APTC_ASSIGNED INT REFERENCES ACC_EMPLOYEE_MAST(AEM_EMP_CODE),
    APTC_TASK_STATUS VARCHAR (1) NOT NULL,
    APTC_TASK_DEPENDENCY INT,
    APTC_TASK_CONTEXT VARCHAR(1) NOT NULL ,
    PRIMARY KEY (APTC_ASSIGNED,APTC_TASK_PER_CODE)
);

CREATE TABLE ACC_PER_TASK_EXP (
    APTE_ASSIGNED_CODE INT REFERENCES ACC_EMPLOYEE_MAST(AEM_EMP_CODE),
    APTE_TASK_PER_CODE INT,
    APTE_TASK_SUMMARY TEXT,
    APTE_TASK_DESCRIPTION TEXT,
    APTE_REMARKS TEXT,
    PRIMARY KEY (APTE_ASSIGNED_CODE,APTE_TASK_PER_CODE)
);

CREATE TABLE ACC_PER_TASK_HIS (
    APTH_ASSIGNED_CODE INT REFERENCES ACC_EMPLOYEE_MAST(AEM_EMP_CODE),
    APTH_TASK_PER_CODE INT,
    APTH_TASK_UPT_DATE TIMESTAMP,
    APTH_TASK_STATUS VARCHAR(1) NOT NULL
);
...
INSERT INTO ACC_EMP_LOGIN (AEL_EMP_CODE,AEL_LOGIN_ID,AEL_LOGIN_PASSWORD,AEL_UPD_DATE) VALUES ((SELECT AEM_EMP_CODE FROM ACC_EMPLOYEE_MAST WHERE AEM_EMP_FIRST_NAME='SuperUser'),'superuser','F68F5A3A90699699EF733516AC84B7E3',NOW());

INSERT INTO ACC_EMPLOYEE_MAST (AEM_EMP_FIRST_NAME,AEM_EMP_CRT_TIME,AEM_EMP_TEAM,AEM_EMP_ROLE,AEM_EMP_JOINING_DATE) VALUES ('admin',NOW(),(SELECT ATM_TEAM_CODE FROM ACC_TEAM_MAST WHERE ATM_TEAM_SHORT_NAME='ADMIN'),'SU',NOW());

INSERT INTO ACC_EMP_LOGIN (AEL_EMP_CODE,AEL_LOGIN_ID,AEL_LOGIN_PASSWORD,AEL_UPD_DATE) VALUES ((SELECT AEM_EMP_CODE FROM ACC_EMPLOYEE_MAST WHERE AEM_EMP_FIRST_NAME='admin'),'admin','4D721D752FE28DA634C8CD3DF6085E4D',NOW());
